...
const Home = () => {
  ...
  if (profile?.role === 'teacher') return <Layout><TeacherView /></Layout>;
  if (profile?.role === 'student') return <Layout><StudentView onSelectCourse={() => {}} onSelectExam={() => {}} /></Layout>;
  if (profile?.role === 'parent') return <Layout><ParentView /></Layout>;
      ...
      <div className="max-w-4xl mx-auto py-12 px-6">
        <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-12 border border-zinc-100 dark:border-zinc-800 shadow-sm">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 font-medium leading-relaxed mb-8">
            You are logged in as a <span className="font-bold text-purple-600 uppercase text-xs">{(profile?.role as string || 'USER').replace('_', ' ')}</span>. 
